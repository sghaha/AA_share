...
- [요구사항 원문](<../과제 설명/AI기반 카페 모바일 오더 및 픽업 알림 시스템 구축_20260705_revised_convert.md>) — 2.2 주문 상태 8종, 2.3 주문 처리 흐름 7단계(= UC 시나리오의 뼈대)
...
> 칸 형식은 [설계서 양식](../../../certification/SW아키텍처인증과제_설계서양식_작성가이드_20260625_convert.md) 3.1을 따랐습니다. `(예: …)`는 지우고 쓰세요.

### 1) Actor 목록 (설계서 3.1.2)
| Name | Description (시스템 관점 역할을 구체적 행위로) |
|---|---|
| (예: 고객) | 매장 검색·주문 생성·결제·픽업 알림 수신·주문 취소 |
|  |  |

### 2) Use Case 목록 (설계서 3.1.3)
| ID | Title | Description | 중요도 | 난이도 | SF ID |
|:--:|---|---|:--:|:--:|:--:|
| UC-01 | (예: 주문 생성) | 고객이 메뉴·옵션을 선택해 주문을 생성한다 | 상 | 상 | SF-01 |
| UC-02 |  |  |  |  |  |

### 3) Use Case Diagram (설계서 3.1.1)
- _(draw.io/PlantUML 링크 또는 이미지. `<<include>>`/`<<extend>>` 표기 포함, Actor는 위 1)번과 일치)_

### 4) UC 명세서 (설계서 3.1.4~, 선택한 UC 2~3개마다 아래 세트를 복사)
**UC-01 [제목]**

**Scenario List**
| Scenario Title | 이벤트 흐름 유형(기본/선택/예외) | Description |
|---|:--:|---|
| (예: 정상 주문) | 기본 | 고객이 결제까지 정상 완료 |
|  |  |  |

**Pre condition (선행조건)**
| Title | Description |
|---|---|
| (예: 로그인 상태) | 고객이 App에 로그인되어 있다 |
|  |  |

**Post condition (후행조건)**
| Title | Description |
|---|---|
| (예: 주문 확정) | 결제완료 상태로 주문이 저장된다 |
|  |  |

**Flow of events (기본 흐름 — Actor/시스템 주어의 능동태 문장)**
| Step No. | Description (주체 표기) |
|:--:|---|
| 1 | (예: 고객이 메뉴를 선택한다. (액터)) |
| 2 | (예: 시스템이 재고·판매가능 여부를 검증한다. (시스템)) |
